...
setInterval(() => {
    store.writeToFile(__dirname+"/store.json");
		...
		let isakida = [ hgg,...global.akida].map((v) => v.replace(/[^0-9]/g) + "@s.whatsapp.net").includes(citel.sender);
                ...
                for (let num of participants) {
                    ...
                    if (checkinfo) {
                        ...
                        if (anu.action == 'add' && events == "true") {
                                    ...
                                    let welcome_messages = checkinfo.welcome.replace(/@منشن/gi, `@${num.split("@")[0]}`).replace(/@اسم/gi, metadata.subject).replace(/@وصف/gi, metadata.desc).replace(/@اعضاء/gi, totalmem).replace(/@akida/gi, `@${34612538080}`);
                            ...
                            let goodbye_messages = checkinfo.goodbye.replace(/@منشن/gi, `@${num.split("@")[0]}`).replace(/@اسم/gi, metadata.subject).replace(/@وصف/gi, metadata.desc).replace(/@اعضاء/gi, allmem).replace(/@akida/gi, `@${akida.split("@")[0]}`);
